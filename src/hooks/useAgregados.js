...
export function useAgregados() {
  ...
  const criar = async (dados) => {
    const { data, error } = await supabase.from('agregados').insert(dados).select().single()
    if (!error) await fetch()
    return { data, error }
  }
 
  const atualizar = async (id, dados) => {
    const { data, error } = await supabase.from('agregados').update(dados).eq('id', id).select().single()
    if (!error) await fetch()
    return { data, error }
  }
